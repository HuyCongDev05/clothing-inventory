...
export function mapBackendSupplierToFrontend(s: BackendSupplierResponse): Supplier {
  return {
    id: String(s.id),
    code: s.code,
    companyName: s.name,
...
export async function createSupplier(form: SupplierFormData, code: string): Promise<Supplier> {
  ...
  const response = await apiFetch<ApiResponse<BackendSupplierResponse>>('/suppliers', {
    method: 'POST',
...
export async function updateSupplier(code: string, form: SupplierFormData): Promise<Supplier> {
  const body = {
    code,
    name: form.companyName,
    contactPerson: form.representative,
  ...
  const response = await apiFetch<ApiResponse<BackendSupplierResponse>>(`/suppliers/${code}`, {
    method: 'PUT',
    body: JSON.stringify(body),
  });
